...
"""Tests de metadata/esquema POR CONEXIÓN Y BASE (Sprint 4). Todo mockeado."""
...
def test_overview_requires_auth(client):
    assert client.get(f"/schema/{CONN}/{DB}/overview").status_code == 401
...
def test_overview_connection_not_found(client):
    ...
    with patch("app.services.connections_repo.get_engine_for_db", return_value=None):
        res = client.get(f"/schema/{CONN}/{DB}/overview", headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 404


def test_overview_ok(client):
    token = _token(client)
    fake = {"server": "PC\\SQLEXPRESS", "database": "MXApp", "table_count": 4, "total_size_kb": 576}
    ...
    assert res.json()["database"] == "MXApp"
...
def test_tables_ok(client):
    ...
    with patch("app.services.connections_repo.get_engine_for_db", return_value=object()), \
        ...
        res = client.get(f"/schema/{CONN}/{DB}/tables", headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 200
...
def test_table_detail_404(client):
    ...
    with patch("app.services.connections_repo.get_engine_for_db", return_value=object()), \
        ...
        res = client.get(f"/schema/{CONN}/{DB}/tables/dbo/nope", headers={"Authorization": f"Bearer {token}"})
    assert res.status_code == 404
